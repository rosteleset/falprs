...
update default_vstream_config set config = config || '{"lpc-net-input-height":224,"lpc-net-input-tensor-name":"input","lpc-net-input-width":224,"lpc-net-model-name":"lpcnet_vit","lpc-net-output-tensor-name":"output","lpr-net-input-height":320,"lpr-net-input-width":320}';
update
  default_vstream_config
set
  config = (
    select jsonb_object_agg(
        key,
        case
            when key like '%-inference-server'
                then to_jsonb(regexp_replace(value #>> '{}', ':[0-9]+$', ':8001'))
            else value
        end
    )
