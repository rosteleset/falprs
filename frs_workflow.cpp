      ...
      } else
      {
        ...
        if (auto char_alpha = url.find('@'); char_alpha != std::string::npos)
        {
          if (auto protocol_suffix = url.find("://"); protocol_suffix != std::string::npos && protocol_suffix < char_alpha)
          {
            ...
            if (auto char_colon = url.find(':', start_of_authority); char_colon != std::string::npos && char_colon < char_alpha)
            {
              auth_user = url.substr(start_of_authority, char_colon - start_of_authority);
              auth_password = url.substr(char_colon + 1, char_alpha - char_colon - 1);
            } else
              auth_user = url.substr(start_of_authority, char_alpha - start_of_authority);
          }
        }
