...
### Обновление проекта
Перед обновлением убедитесь, что у вас есть актуальный бэкап баз данных проекта. Переменные окружения берутся из упомянутого выше *.env* файла. Обновите репозиторий и используйте скрипт **scripts/update_falprs.sh**:
```bash
cd ~/falprs
git pull
...
```
Что делает скрипт:
* останавливает сервиc falprs
* останавливает контейнер с Triton Inference Server
* собирает проект с помощью скрипта *scripts/build_falprs.sh*
* создаёт TensorRT планы моделей нейронных сетей с помощью скрипта *scripts/build_falprs.sh*
* запускает контейнер с Triton Inference Server
* обновляет схемы и данные в БД (старые данные не затираются)
* запускает сервис falprs
