...
* Строгое соблюдение типов данных в [API](https://rosteleset.github.io/falprs/) запросах. Например, если ожидается числовое поле, то его нельзя заключать в кавычки.
...
* [Сборка и настройка проекта](#build_and_setup_falprs)
   ...
   * [Инициализация PostgreSQL](#init_postgresql)
...
* [Обновление проекта](#update_falprs)
* [Примеры](#examples)
   * [LPRS](#lprs_examples)
   * [FRS](#frs_examples)
* [Тесты](#tests)
...
* Ubuntu 24.04 LTS или Ubuntu 26.04 LTS.
...
Большинство скриптов репозитория используют переменные окружения. В директории *scripts* сделайте копию файла *.env.example* и назовите *.env*
Укажите актуальные значения для вашей системы.
Мажорная версия PostgreSQL задаётся переменной **PG_VERSION**. Установленную версию PostgreSQL можно узнать командой:
```bash
psql --version
psql (PostgreSQL) 16.14 (Ubuntu 16.14-0ubuntu0.24.04.1)
```
Значит, *PG_VERSION=16*
Остальные переменные с префиксом **PG_** определяют подключение к PostgreSQL. Не забудьте указать свой пароль.
Версия контейнера с Triton Inference Server задаётся переменной **TRITON_VERSION**. Определяется вычислительной способностью (**Compute Capabilty**) — версией аппаратной архитектуры NVIDIA GPU, которая определяет набор доступных инструкций, поддерживаемые технологии (например, **Tensor-ядра**) и физические ограничения графического процессора. Она необходима для **CUDA-компилятора** и ПО, чтобы понимать, как эффективно выполнять код на видеокарте. Узнать Compute Capability можно командой:
```bash
nvidia-smi --query-gpu=compute_cap --format=csv
```
#### Таблица поддержки Compute Capability и версии контейнера
|Compute Capability|Архитектура GPU|Версия контейнера|
|--|--|--|
|< 6.0|Maxwell, Kepler, Fermi |Не поддерживается|
|6.x|Pascal|24.04|
|7.0|Volta|24.09|
...
**Важно:** если вы делаете миграцию с устаревшего проекта FRS, то необходимо с помощью утилиты *sha1sum* указать хеш-сумму ONNX файла, который вы использовали для создания TensorRT плана старой модели *arcface*. Это значение указывается в переменной **ARCFACE_SHA1**. Например, подсчёт хеш-суммы для старого файла *glint_r50.onnx*:
```bash
sha1sum glint_r50.onnx
4fd7dce20b6987ba89910eda8614a33eb3593216  glint_r50.onnx
```
Тогда *ARCFACE_SHA1=4fd7dce20b6987ba89910eda8614a33eb3593216*

<a id="init_postgresql"></a>
### Инициализация PostgreSQL
Используйте скрипт **scripts/init_postgresql.sh**:
```bash
sudo ~/falprs/scripts/init_postgresql.sh
```
Скрипт создаёт базы данных, пользователей и назначает им роли.

<a id="build_falprs"></a>
### Сборка проекта
Используйте скрипт **scripts/build_falprs.sh**:
```bash
...
Для инференса используются TensorRT планы, которые могут быть получены из моделей нейронных сетей в формате ONNX (Open Neural Network Exchange). Для создания планов в рабочей директории используйте скрипт **scripts/tensorrt_plans.sh**:
```bash
...
### Обновление проекта
Перед обновлением убедитесь, что у вас есть актуальная резервная копия баз данных проекта. Переменные окружения загружаются из упомянутого выше файла *.env*.

**Важно:** если вы делаете обновление со старой версии, в которой не использовался файл *.env*, то вам сначала следует ознакомиться с разделом [Переменные окружения](#env_vars). Значение переменной **TRITON_VERSION** укажите в зависимости от того, какая рабочая версия у вас используется сейчас.

Обновите репозиторий и запустите скрипт **scripts/update_falprs.sh**:
```bash
cd ~/falprs
git pull
...
```
Скрипт:
* проверяет, требуется ли пересборка установленного исполняемого файла FALPRS, и выполняет её только в случае, если установленная версия отличается от версии проекта или не может быть определена
* до остановки каких-либо сервисов выполняет планирование генерации TensorRT и определяет, требуется ли пересоздание каких-либо TensorRT планов
* загружает необходимые модели нейронных сетей и проверяет их SHA1-хэши
* пересоздаёт TensorRT планы только для изменившихся моделей или отсутствующих plans
* останавливает сервис `falprs` и контейнер Triton Inference Server только в том случае, если требуется генерация TensorRT планов
* после успешной генерации TensorRT планов повторно запускает контейнер Triton Inference Server, если он был запущен до обновления
* обновляет схемы и данные в БД (старые данные не теряются)
* после обновления восстанавливает исходное состояние сервиса `falprs`

Если исполняемый файл FALPRS актуален и все TensorRT планы соответствуют текущим моделям, сервис `falprs` и контейнер Triton Inference Server не останавливаются во время обновления.
