...
В директории *scripts* сделайте копию файла *.env.example* и назовите *.env*
...
Остальные переменные с префиксом **PG_** определяют подключение к PostgreSQL. Не забудьте указать пароль, который вы использовали при выполнении SQL команд на предыдущем шаге.
...
<a id="build_falprs"></a>
### Сборка проекта
Для сборки проекта используйте скрипт **scripts/build_falprs.sh**:
```bash
...
```
Скрипт:
* проверяет, требуется ли пересборка установленного executable FALPRS, и выполняет её только в случае, если установленная версия отличается от версии проекта или не может быть определена
...
Если executable FALPRS актуален и все TensorRT планы соответствуют текущим моделям, сервис `falprs` и контейнер Triton Inference Server не останавливаются во время обновления.
