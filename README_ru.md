...
### Обновление проекта
Перед обновлением убедитесь, что у вас есть актуальный бэкап баз данных проекта. Переменные окружения берутся из упомянутого ваше *.env* файла. Для обновления используйте скрипт **scripts/update_falprs.sh**:
```bash
