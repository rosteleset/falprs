...
# arcface - glint360k_r50.onnx
wget --content-disposition --no-clobber 'https://drive.usercontent.google.com/download?id=1aO2QfGAd8cVsZ-V-X5Wb8YJsm0BdscRT&confirm=y'
...
docker run --gpus all --rm -v $BASEDIR/../temp:/source -v $FALPRS_WORKDIR/model_repository:/destination --entrypoint=bash nvcr.io/nvidia/tritonserver:$TRITON_VERSION-py3 -c '
	/usr/src/tensorrt/bin/trtexec --onnx=/source/glint360k_r50.onnx --saveEngine=/destination/arcface/1/model.plan --shapes=input.1:1x3x112x112
